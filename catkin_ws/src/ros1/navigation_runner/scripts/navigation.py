...
class Navigation:
    ...
    def goal_callback(self, goal):
        ...
        prev_target_dir = self.target_dir.clone() if self.target_dir is not None else None
        ...
        if not had_goal or prev_target_dir is None:
            self.stable_times = 0
            return

        prev_dir_2d = prev_target_dir[:2]
        curr_dir_2d = self.target_dir[:2]
        prev_norm = torch.norm(prev_dir_2d)
        curr_norm = torch.norm(curr_dir_2d)
        if prev_norm < 1e-3 or curr_norm < 1e-3:
            return

        cos_angle = torch.clamp(torch.dot(prev_dir_2d, curr_dir_2d) / (prev_norm * curr_norm), -1.0, 1.0)
        if torch.acos(cos_angle).item() > 0.35:
            self.stable_times = 0
